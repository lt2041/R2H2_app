...
## Assumptions 
This model assumes the fuel cell operates at 80°C (353.15K) which directly affects the pressure values. This is further discussed in **"Partial Pressures"**
...
This is the theoretical cell voltage based on temperature, pressure and reactant concentration.
...
This is the voltage lost from the electrochemical reactions requiring energy to overcome their reaction barriers.

Gas concentration is calculated using the ideal gas law:
>```
...
This is the voltage lost due to resistance to the flow of electrons through the fuel cell.
...
This is the voltage lost when reactants cannot reach the electrode reaction sites quickly enough, causing depletion at high current densities.
...
### Partial Pressures
Partial pressure of oxygen and water vary depending on the capabilities of the fuel cell. Pressurised (3 atm), 80°C, 100% RH:
- Oxygen, 0.531 atm = `5.38e4` Pa
- Water, 0.47 atm = `4.76e4` Pa

This system sets hydrogen inlet pressure to 3 bar (`3.0e5` Pa)

These values can likely be changed with greater research into the subject. For reference, see Amphlett et al. for more<sup>[5]</sup>.
...
- `k_cycle = 1e-3` is currently unsourced and is entirely estimated.
...
The random step simulation is a sequence of random current density steps generated by `generate_random_transitions`, each with a random size and duration within set bounds, and stitched into a smooth commanded profile `current_profile` using smoothing between steps.
...
**NOTE: This simulation is only used to show dynamic output of the fuel cell and is not crucial for future use within R2H2.**
...
- Amphlett et al. https://www.sciencedirect.com/science/chapter/edited-volume/pii/B978032399485900006X
