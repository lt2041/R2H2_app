...
V1.1.3 - main() now uses the RANDOM STEP SIMULATION SETTINGS constants (previously hardcoded locally),
         and generate_random_transitions() defaults now reference those same constants.
V1.1.4 - Final commit within internship. Made minor tweaks to values to align with literature and README.
"""
...
#
# Everything within this section is used to create random
# steps of power output within the simulation.
# =========================================================
...
# =========================================================
# SIMULATE RANDOM STEP TRANSITIONS
# =========================================================


def generate_random_transitions(n_steps=N_TRANSITIONS, J_min=0.0, J_max=RATED_CURRENT_DENSITY,
                                 max_dJ=TRANSITION_MAX_DJ, min_dt=TRANSITION_MIN_DT, max_dt=TRANSITION_MAX_DT):
    transitions = []
    t = 0.0
    J_now = 0.0
...
def build_raw_transition_profile(transitions, dt=SIM_DT, t_end=SIM_T_END):
...
def plot_polarisation_curves(cell):
    ...
    # Marker: cell voltage at rated current density
    V_rated = cell.get_voltage(cell.rI_rated)
    plt.plot(cell.rI_rated, V_rated, 'o', color='red', markersize=8, zorder=5,
              label=f"Rated ({cell.rI_rated:.2f} A/cm², {V_rated:.3f} V)")
    
    plt.xlabel("Current Density (A/cm²)")
    plt.ylabel("Cell Voltage (V)")
    plt.title("PEMFC Polarisation Curve")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def plot_power_density_curve(cell):
    ...
    # Marker: power density at rated current density
    Pd_rated = cell.get_voltage(cell.rI_rated) * cell.rI_rated
    plt.plot(cell.rI_rated, Pd_rated, 'o', color='red', markersize=8, zorder=5,
              label=f"Rated ({cell.rI_rated:.2f} A/cm², {Pd_rated:.3f} W/cm²)")

    # Marker: peak power density point
    idx_max = np.argmax(cell.arP_density)
    J_peak = cell.arCurrentDensity[idx_max]
    Pd_peak = cell.arP_density[idx_max]
    plt.plot(J_peak, Pd_peak, '^', color='green', markersize=9, zorder=5,
              label=f"Peak ({J_peak:.2f} A/cm², {Pd_peak:.3f} W/cm²)")
...
def main():
    ...
    transitions = generate_random_transitions(n_steps=N_TRANSITIONS)

    time_raw, J_raw = build_raw_transition_profile(transitions, dt=SIM_DT, t_end=SIM_T_END)

    # ============================================================
    # PLOT RAW + SMOOTHED PROFILES FOR tau values
    # ============================================================

    taus_to_plot = TAUS_TO_PLOT
    dt = SIM_DT
    v_load = V_LOAD_LIMIT
    ...
    # top row of outputs table
    print(f"{'Tau':>5} | {'H2 Used (t)':>12} | {'Energy (MWh)':>14} | "
        f"{'kWh/kg of H2 (MWh)':>12} | {'Δ(kWh/kg)':>12} | {'Eff (%)':>10} | {'Degradation (µV)':>17}")
    print("-" * 105)
