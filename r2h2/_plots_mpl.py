...
r2h2._plots_mpl
===============
Matplotlib backend for r2h2 standard results plots.
...
def plot_hourly_overview(sim, out, year=0, title=None, save_path=None):
    plt = _plt()
    ...
    fig.suptitle(title or f"R2H2 Simulation – hourly overview (year {year + 1})", fontsize=14)
...
def plot_degradation(out, year=0, title=None, save_path=None):
    plt = _plt()
...
def plot_battery_fade(out, year=0, title=None, save_path=None):
    plt = _plt()
...
def plot_second_traces(sim, hour=0, title=None, save_path=None):
    plt = _plt()
...
# ---------------------------------------------------------------------------
# 5.  Per-bank temperatures for a single hour
# ---------------------------------------------------------------------------

def plot_bank_temperatures(sim, hour=0, title=None, save_path=None):
    plt = _plt()
    from r2h2.r2h2 import runElectroStackStep1, bank_thermal_from_kind
...
def plot_multi_year_summary(out, title=None, save_path=None):
    plt = _plt()
...
def plot_all(sim, out, year=0, save_dir=None) -> dict:
    ...
    figs: dict = {}
    figs["hourly_overview"] = plot_hourly_overview(sim, out, year=year, save_path=_path("hourly_overview"))
    figs["degradation"]     = plot_degradation(out, year=year, save_path=_path("degradation"))
    figs["battery_fade"]    = plot_battery_fade(out, year=year, save_path=_path("battery_fade"))
    ...
    if len(out["YearResults"]) > 1:
        figs["multi_year_summary"] = plot_multi_year_summary(out, save_path=_path("multi_year_summary"))
